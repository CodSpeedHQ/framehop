        ...
        let (new_lr, new_sp, new_fp) = match self {
            ...
            UnwindRuleAarch64::NoOpIfFirstFrameOtherwiseFp => {
                if is_first_frame {
                    ...
                    // `new_sp = fp + 16` is only an estimate of the caller's SP; a
                    // subsequent SP-relative DWARF rule must treat it as such.
                    regs.set_sp_unreliable(true);
            ...
            UnwindRuleAarch64::OffsetSpAndRestoreFpAndLr {
                ...
                // If we reached this frame with an SP that is only a
                // frame-pointer-derived estimate — i.e. the caller frame had no
                // unwind info and was unwound via the frame pointer, where
                // `new_sp = fp + 16` is only an approximation of the caller's SP
                // (common for Chromium/Electron, which ship almost no `.eh_frame`)
                // — then evaluating this SP-relative DWARF rule with that SP reads
                // the wrong stack slots and produces a bogus return address. That
                // surfaces as a garbage, unsymbolicated root frame.
                //
                // For a frame-based function the slot holding the caller's FP is
                // exactly where this function's own FP register points, so the
                // invariant `sp + fp_offset == fp` must hold. When SP is unreliable
                // and the invariant is violated, re-anchor SP on the (reliable) FP
                // so the rest of the rule reads from the correct frame record.
                let sp = if regs.sp_is_unreliable()
                    && fp != 0
                ...
                // EXPERIMENT: 4.17.0 FP=0 end-of-stack check temporarily disabled to
                // test whether it is still necessary alongside the SP re-anchoring fix.
                // if new_fp == 0 {
                //     return Ok(None);
                // }
                // The SP used here is precise (either it was already reliable, or
                // we just re-anchored it on the frame pointer), so `new_sp` is too.
                regs.set_sp_unreliable(false);
                (new_lr, new_sp, new_fp)
            }
            UnwindRuleAarch64::UseFramePointer => {
            ...
            UnwindRuleAarch64::UseFramepointerWithOffsets {
                ...
                // This rule comes from an fp-relative DWARF CFA, so `new_sp` is precise.
                regs.set_sp_unreliable(false);
                (new_lr, new_sp, new_fp)
            }
        };
